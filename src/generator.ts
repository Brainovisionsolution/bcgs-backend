...
import fs from 'fs'
import { calculateDuration, getGenderPronoun } from './utils'
import dayjs from 'dayjs'

export async function generateCertificatePDF(certData: any, template: any, bgBase64?: string, logoBase64?: string) {
  // 1. Generate QR Code
  ...
  const durationText = certData.duration ? certData.duration : calculateDuration(certData.start_date, certData.end_date)
  const pronoun = getGenderPronoun(certData.gender)
  ...
  const dataMapping: any = {
    ...
    "START_DATE": dayjs(certData.start_date).format('DD MMM YYYY'),
    "END_DATE": dayjs(certData.end_date).format('DD MMM YYYY'),
    "DATE": dayjs(certData.issue_date).format('DD/MM/YYYY'),
  ...
  for (const [key, p] of Object.entries(template.layout_json as Record<string, any>)) {
    // Skip special non-text keys
    if (['WORD_TEMPLATE', 'QR_CODE', 'QR_POSITION', 'QR_SIZE', 'SIGNATURE', 'SEAL'].includes(key)) continue;
  ...
  if (qrPositionStr) {
    const margin = 60;
    if (qrPositionStr === 'BOTTOM_RIGHT') qrPos = { x: pdfWidth - qrSize - margin, y: pdfHeight - qrSize - margin, width: qrSize };
    else if (qrPositionStr === 'BOTTOM_LEFT') qrPos = { x: margin, y: pdfHeight - qrSize - margin, width: qrSize };
    else if (qrPositionStr === 'BOTTOM_MIDDLE') qrPos = { x: (pdfWidth - qrSize) / 2, y: pdfHeight - qrSize - margin, width: qrSize };
    else if (qrPositionStr === 'MIDDLE_RIGHT') qrPos = { x: pdfWidth - qrSize - margin, y: (pdfHeight - qrSize) / 2, width: qrSize };
    else if (qrPositionStr === 'MIDDLE_LEFT') qrPos = { x: margin, y: (pdfHeight - qrSize) / 2, width: qrSize };
    else if (qrPositionStr === 'TOP_RIGHT') qrPos = { x: pdfWidth - qrSize - margin, y: margin, width: qrSize };
    else if (qrPositionStr === 'TOP_LEFT') qrPos = { x: margin, y: margin, width: qrSize };
  }
  ...
  const pdfPath = path.join(generatedDir, `${documentId}.pdf`)
